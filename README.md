...
3. Change its seven values:
   ...
   - `displayFromHebrewDate`: the first Hebrew date on which today's app view may show the entry.
   - `displayUntilHebrewDate`: the last Hebrew date on which today's app view may show the entry.
4. Commit to `main`. GitHub checks the file before deploying it.

The display range is inclusive and must run forward. It controls whether the entry is visible based
on today's Hebrew date; it does not change the yahrzeit date or its yearly recurrence.
